...
const VotingsShow = ({
  ...
  const filteredData = dummyData.filter((data) => {
    ...
    const matchesCategory =
      ...
      category.length === 0 ||
      category.includes(data.category.toString());
    const matchesGenre =
      genre.includes("all") ||
      genre.length === 0 ||
      genre.includes(data.genre.toString());
  ...
  const sortedData = filteredData.sort((a, b) => {
    switch (sortBy) {
      ...
      case "leastVotes":
        return a.positiveVotes! - b.positiveVotes!;
      case "mostRecent":
  ...
  return (
    <div className="flex flex-wrap items-center justify-center w-full gap-6 md:gap-12 lg:gap-16">
      {sortedData.length > 0 ? (
        sortedData.map((data) => (
          <SubmissionCard key={data.id} version="buttons" submission={data} />
        ))
      ) : (
        <h3 className="font-content-lg mt-6">No results Found!</h3>
      )}
    </div>
  );
};

export default VotingsShow;
